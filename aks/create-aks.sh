...
# this might be all thats needed, might not need the operator
kubectl apply -f - <<EOF
apiVersion: apps/v1
kind: DaemonSet
metadata:
...
helm upgrade --install prometheus -n monitoring --create-namespace prometheus-community/prometheus
helm upgrade --install grafana -n monitoring --create-namespace grafana/grafana
